<script>
	...
	import { userData } from "$lib/stores/global";
  ...
  const login = () => {
    ...
    if(!uName.match($userData.username)) {
      uNameErr = true
      uNameErrMsg = 'Username cannot be found'
    }
    
    if(!pWord.match($userData.password)) {
      pWordErr = true
      pWordErrMsg = 'Wrong password'
    }
    ...
    if(uName.match($userData.username) && pWord.match($userData.password)) {
      goto(`/${uName}`, {replaceState: true})
    }
  }
...
<div class="w-full h-full flex flex-col items-center gap-y-5">
  ...
  <div class="w-full flex flex-col items-center gap-y-3 mt-10">
    ...
    <div class="w-[206px] flex flex-col">
      ...
      <input bind:value={pWord} class="w-full h-[37px] rounded-lg pl-3 lexend text-xs" type='text'>
      {#if pWordErr}
        <div class="lexend text-paleRed text-[10px]">
          {pWordErrMsg}
        </div>
  ...
  <div class="w-full flex flex-wrap justify-center items-center mt-20 gap-y-2">
